/*!
 *  Ohjelma lukee salama-dataa tiedostosta ja muodostaa siitä queryDatan
 *  ja tulostaa sen stdout:iin.
 *
 *  Annettu salama data pitää olla muodossa:
 ...
 *  jne.
 *
 *  Kyseistä dataa voi hakea vaikka tietokannasta seuraavalla SQL-kyselyllä:
 *
 *  SELECT
...
bool ParseFlashDataLine(const std::string &theLineStr, FlashData &theData)
{
  if (!theLineStr.empty())
  {
    ...
    if (words.size() == 6)
    {
      ...
      throw runtime_error(std::string("Data rivillä oli väärä määrä arvoja: \n") + theLineStr);
  }
  return false;
}
                                    ...
                                    bool fMakeLocal2UtcTimeConversion)
{
  ...
  if (!theFlashStrings.empty())
  {
    ...
    if (data)
    {
      ...
      info.First();
      // Täytetään lopuksi eri parametrit infoon/dataan
