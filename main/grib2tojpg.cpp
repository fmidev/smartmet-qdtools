...
//
// Nämä kulkevat toistaiseksi globaaleissa muuttujissa,
// koska muuten koodia pitäisi refaktoroida runsaasti.
//
// Tämä koodi on menossa refaktoroitavaksi myöhemmin,
// joten siitä pitää ottaa ko. asiat huomioon
                                   ...
                                   NFmiPoint &theHybridDataGridSize)
{
  ...
  if (pos != string::npos)
  {
    ...
    if (pos != string::npos)
    {  // eli löytyi 2. ':' -merkki, joka merkitsee että annetaan halutun hilan koko
      // nyt pitää tutkia, onko annettu useita hilakokoja eli onko annettu myös
      ...
      {  // nyt on annettu yksi ylimääräinen hila koko
        gridSizeFunction =
            0;  // laitetaan error koodi päälle ja korjataan se myöhemmin pois jos homma onnistui
        string::size_type pos2 = theProjStr.find(",", pos + 1);
        if (pos2 != string::npos)
        {
          pos2 = theProjStr.find(",", pos2 + 1);
          if (pos2 != string::npos)
          {  // nyt löytyi toinen pilkku, josta alkaa ylimääräisten hilakokojen määritys. Tämä loppu
            // pitää saada
            // pois hilan määritys stringistä
...
int main(int argc, const char **argv)
{
  ...
  NFmiLevelBag
      ignoredLevelList;  // lista missä yksittäisiä leveleitä, mitkä halutaan jättää pois laskuista
  vector<NFmiQueryData *> datas;
  vector<FmiLevelType> acceptOnlyLevelTypes;  // lista jossa ainoat hyväksyttävät level typet
...
#if 0
	  bool useLevelTypeFileNaming = false; // n optiolla voidaan laittaa output tiedostojen nimen perään esim. _levelType_100
			...
			for(int i=0; i<ssize; i++)
			{
				...
				if(fUseOutputFile)
				{
					...
					if(useLevelTypeFileNaming)
					{
						...
						// TÄMÄ PITÄÄ vielä korjata, jos saman level tyypillä on erilaisia hila/area määrityksiä, pitää ne nimetä!!!
				...
				else if(ssize > 1) // jos qdatoja syntyy enemmän kuin 1, pitää antaa output-tiedoston nimi, että muut tiedostonimet voidaan generoida
				{
					cerr << "GRIB:istä syntyy useita eri sqd-data tiedostoja." << endl;
					cerr << "Käytä o-optiota antamaan yhden tiedoston nimi (muut nimet" << endl;
...
static NFmiArea *CreateLatlonArea(grib_handle *theGribHandle, bool &doGlobeFix)
{
  ...
  grib_get_double(
      theGribHandle,
      "grib1divider",
      &grib1divider);  // katsotaan jos löytyy grib-divider, muuten käytetään defaultti arvoa
...
{
        long gridDefinitionTemplateNumber = 0; // tähän tulee projektio tyyppi, ks. qooglesta (grib2
Table 3.1)
...
{
  /* // käytetäänkin aina ktegoria+paramnumber kikkaa
          long parameter = 0;
  ...
  */
  {  // jos ei löytynyt normaalia reittiä parametri arvoja, kokeillaan parameterCategory +
     // parameterNumber yhdistelmää
    ...
    {  // tehdään parametri numeroksi categorian ja numberin yhdistelmä niin että eri categorioissa
      // olevat
      // parametrit eivät menisi päällekkäin eli parId = 1000*categoria+number
                         ...
                         vector<ParamChangeItem> &theParamChangeTable)
{
  ...
  if (status1 == 0 && status2 == 0)
  {
    ...
    if (status4 == 0)
    {
      ...
      else if (scanningMode == 80)  // en tiedä mikä tämä moodi on, grib2 speksi ei määrää 5. tai 7.
                                    // bitin merkitystä
      // jouduin vain kokeilemaan LAPS datan juoksutuksen käsipelillä
      {
        for (size_t i = 0; i < values_length; i++)
        {
          if (doubleValues[i] == theGridRecordData->itsMissingValue)
    ...
    if (doGlobeFix)
    {
      ...
      // Eli matriisissa olevia arvoja pitää siirtää niin että vasemmalla puoliskolla olevat
      // laitetaan oikealle
    ...
    {  // tässä tehdään latlon projisointia
...
static_cast<float>(doubleValues[i]);
                ...
                if(doGlobeFix)
                {
                        // Nyt on siis tilanne että halutaan 'korjata' globaali data editoria
varten.
                        // Latlon-area peittää maapallon longitudeissa 0-360, se pitää muuttaa
editoria varten longitudeihin -180 - 180.
                        // Eli matriisissa olevia arvoja pitää siirtää niin että vasemmalla
                                          ...
                                          const NFmiPoint &theHybridDataGridSize)
{
  ...
  try
  {
           ...
           err != GRIB_SUCCESS)
    {
      ...
      //				continue; // ***** POISTA TÄMÄ, TESTI VAIHEESSA PURETAAN
      // VAIN max 10 kenttää!!!!  *******
      //			if(counter % 30 != 0)
      //				continue;
      ...
      try
      {
        ...
        /*
                                        if(counter == 1)
                                                firstValidTime = tmpData->itsValidTime; // Tämä
           validtime juttu oli vain testaamista varten gfs grib2 datasta, koska siellä oli joku
           kenttä toisessa ajassa kuin muut (kun tiedostossa pitäisi olla vain yhden aika-askeleen
           dataa)
...
void CheckInfoSize(const NFmiQueryInfo &theInfo, int theMaxQDataSizeInBytes)
{
  ...
  if (theMaxQDataSizeInBytes < static_cast<int>(infoSizeInBytes))
  {
    ...
    ss << "Hilapisteitä/asemia on " << locSize << " kpl." << endl;
    unsigned long levelSize = theInfo.SizeLevels();
    ss << "Leveleitä on " << levelSize << " kpl." << endl;
