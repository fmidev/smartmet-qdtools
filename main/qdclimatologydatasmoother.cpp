...
void Usage(void)
{
  ...
  cout << "the calculation time. Values are taken from those days only from the same utc hour."
       << endl;
...
float CalcSmoothedValue(NFmiFastQueryInfo &sourceInfo,
                        const std::vector<unsigned long> &timeIndices)
{
...
static std::vector<unsigned long> MakeTimeIndexVector(NFmiFastQueryInfo &info, int smoothedDays)
{
  ...
  for (int i = 0; i < smoothedDays; i++, wantedTime.ChangeByDays(1))
  {
    ...
    else
    {
      ...
      // Remember that we have year long climatology data, and we have to look data over the ends
      // from the other side of data.
      ...
      else
      {
        ...
        {
          std::cout << "\nNon found time with time-index " << originalTimeIndex << " and time "
                    << wantedTime << ", error in data?" << std::endl;
          //                    ::MakeErrorAndUsageMessages("Error: It seems that given data doesn't
          //                    have the right year long data set, stopping execution...");
        }
      }
    }
  }
...
static void FillOneTimeStep(NFmiFastQueryInfo &destinationInfo,
                            NFmiFastQueryInfo &sourceInfo,
                            const std::vector<unsigned long> &timeIndices)
{
  for (destinationInfo.ResetParam(), sourceInfo.ResetParam();
       destinationInfo.NextParam() && sourceInfo.NextParam();)
  {
    for (destinationInfo.ResetLevel(), sourceInfo.ResetLevel();
         destinationInfo.NextLevel() && sourceInfo.NextLevel();)
    {
      for (destinationInfo.ResetLocation(), sourceInfo.ResetLocation();
           destinationInfo.NextLocation() && sourceInfo.NextLocation();)
      {
                        ...
                        int smoothedDays)
{
  ...
  sourceInfo.First();
  // Time must be the out most loop, because we have to calculate for each time its own timeIndex
  // list
...
static NFmiTimePerioid CalcTimeListResolution(NFmiTimeList &timeList)
{
  ...
  auto maxElement = max_element(timeDiffInMinutesCounter.begin(),
                                timeDiffInMinutesCounter.end(),
...
static NFmiQueryInfo MakeNewMetaInfo(NFmiFastQueryInfo &info)
{
  ...
  else
  {
    ...
    return NFmiQueryInfo(
        info.ParamDescriptor(), times, info.HPlaceDescriptor(), info.VPlaceDescriptor());
  }
}
  ...
  if (cmdLine.isOption('d'))
  {
    ...
    if (smoothedDays < 1)
      ::MakeErrorAndUsageMessages(
          "Error: Invalid smoothed-days value given, must be positive odd number and at least 1");
    if (smoothedDays % 2 == 0)
      ::MakeErrorAndUsageMessages("Error: Invalid smoothed-days value given, must be odd number");
    if (smoothedDays > 50)
      ::MakeErrorAndUsageMessages(
          "Error: Invalid smoothed-days value given, can't be over 50 days, that would give "
          "questianable results");
  }
  return smoothedDays;
}
...
static NFmiProducer GetProducer(NFmiCmdLine &cmdLine)
{
  ...
  if (cmdLine.isOption('p'))
  {
    ...
    if (strVector.size() != 2)
      ::MakeErrorAndUsageMessages(
          "Error: Error: with -p option, 2 comma separated parameters expected (e.g. 240,Ec)");
