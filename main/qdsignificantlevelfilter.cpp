...
static void CheckData(NFmiFastQueryInfo &info,
                      const string &filePath,
                      bool needToHaveSignificantParam)
{
  if (info.IsGrid())
    MakeCheckDataError(
        filePath, "Error: input data was grid data, can't remove non-significant levels with it.");
...
static unsigned long FindMaxLevelCount(const SignificantLevelsMap &levelsMap)
{
  ...
  for (const auto &levels : levelsMap)
  {
    if (levels.second)
    {
      ...
      if (maxLevelCount < tmpLevelCount) maxLevelCount = tmpLevelCount;
    }
  }
...
static NFmiQueryInfo MakeNewMetaInfo(NFmiFastQueryInfo &info, unsigned long newLevelSize)
{
  ...
  return NFmiQueryInfo(
      info.ParamDescriptor(), info.TimeDescriptor(), info.HPlaceDescriptor(), levelDescriptor);
}

static unique_ptr<NFmiQueryData> CreateNewEmtyData(NFmiFastQueryInfo &info,
                                                   const SignificantLevelsMap &levelsMap)
{
...
static void FillSignificantLevelData(
    NFmiFastQueryInfo &sourceInfo,
    NFmiFastQueryInfo &destInfo,
    const NFmiQueryDataUtil::SignificantSoundingLevels &levelIndicesPtr)
{
  if (levelIndicesPtr)
  {
...
static void FillNewData(const unique_ptr<NFmiQueryData> &newData,
                        NFmiFastQueryInfo &sourceInfo,
...
// T‰m‰ funktio piti tehd‰, koska NFmiHPlaceDescriptor:in Combine ei tarkastele mielest‰ni oikein
// asemien
// samanlaisuutta. Se tutkii vain onko asema saman niminen ja onko sill‰ tarkalleen samat
// koordinaatit.
// Min‰ taas haen sit‰ ett‰ lˆytyykˆ asemilta sama id numero (nimet on saatettu hakea erilaisilla
// par-tiedotoilla
...
static NFmiHPlaceDescriptor MakeCombinedLocationsByIdent(NFmiHPlaceDescriptor locations1,
                                                         NFmiHPlaceDescriptor locations2)
{
  auto lessThanLocation = [](const NFmiLocation *location1, const NFmiLocation *location2)
  {
    return location1->GetIdent() < location2->GetIdent();
  };
...
static bool IsNotTotalWindSubParam(const NFmiDataIdent &dataIdent)
{
  ...
  if (paramIdent != kFmiWindDirection && paramIdent != kFmiWindSpeedMS &&
      paramIdent != kFmiWindUMS && paramIdent != kFmiWindVMS)
    return true;
  else
    return false;
}
...
{
#if 0
  auto equalParam = [](const NFmiDataIdent &dataIdent1, const NFmiDataIdent &dataIdent2)
  {
    return dataIdent1.GetParamIdent() == dataIdent2.GetParamIdent();
  };
#endif
  ...
  if (containsTd1 && containsTd2) uniqueParams.erase(Td2);
}

// Sama juttu parametrien kanssa kuin locationien.
// Tulos dataan laitetaan aina TotalWind parametri ja siksi siit‰ on poistettava sen aliparametrit
// p‰‰tasolta.
// Tulosdataan laitetaan
static NFmiParamDescriptor MakeCombinedParamsByIdent(NFmiParamDescriptor params1,
                                                     NFmiParamDescriptor params2)
{
  auto lessThanParam = [](const NFmiDataIdent &dataIdent1, const NFmiDataIdent &dataIdent2)
  {
    return dataIdent1.GetParamIdent() < dataIdent2.GetParamIdent();
  };
...
static bool HasAnyDataAtThatTime(NFmiFastQueryInfo &info, const NFmiMetTime &time)
{
  if (info.Time(time))
  {
    // Riitt‰‰ kun tutkitaan ett‰ lˆytyykˆ mit‰‰n arvoja paina parametrista milt‰‰n asemalta milt‰‰n
    // levelilt‰
    if (info.Param(kFmiPressure))
    {
      for (info.ResetLocation(); info.NextLocation();)
      {
        ...
        {
          if (info.FloatValue() != kFloatMissing) return true;
        }
      }
    }
  }
...
// K‰yd‰‰n jokainen aika l‰pi molemmista datoista ja jos yht‰t‰‰n luotaus dataa ei lˆydy
// kummastakaan datasta
// millek‰‰n asemalle, poistetaan kyseinen aika listasta.
static NFmiTimeDescriptor MakeCombinedTimesWithRealData(NFmiQueryData &filteredData,
                                                        NFmiQueryData &lowResolutionData)
{
  ...
  {
    if (::HasAnyDataAtThatTime(filteredInfo, time) ||
        ::HasAnyDataAtThatTime(lowResolutionInfo, time))
      timesWithData.Add(new NFmiMetTime(time), false, true);
  }
  return NFmiTimeDescriptor(filteredInfo.OriginTime(), timesWithData);
}

static NFmiQueryInfo MakeCombinedMetaInfo(NFmiQueryData &filteredData,
                                          NFmiQueryData &lowResolutionData)
{
  auto &filteredInfo = *filteredData.Info();
  auto &lowResolutionInfo = *lowResolutionData.Info();

  NFmiParamDescriptor combinedParams = ::MakeCombinedParamsByIdent(
      filteredInfo.ParamDescriptor(), lowResolutionInfo.ParamDescriptor());
  NFmiHPlaceDescriptor combinedLocations = ::MakeCombinedLocationsByIdent(
      filteredInfo.HPlaceDescriptor(), lowResolutionInfo.HPlaceDescriptor());
  NFmiTimeDescriptor combinedTimes =
      ::MakeCombinedTimesWithRealData(filteredData, lowResolutionData);
  // Kummassa datassa on enemm‰n leveleit‰, otetaan pohjaksi sellaisenaan (sounding levelit ovat
  // vain j‰rjestysnumeroita).
  NFmiVPlaceDescriptor combinedLevels =
      (filteredInfo.SizeLevels() >= lowResolutionInfo.SizeLevels())
          ? filteredInfo.VPlaceDescriptor()
          : lowResolutionInfo.VPlaceDescriptor();
...
{
  bool anyDataFilled = false;
  for (combinedInfo.ResetLevel(), sourceInfo.ResetLevel();
       combinedInfo.NextLevel() && sourceInfo.NextLevel();)
  {
    ::SetValue(combinedInfo, sourceInfo, anyDataFilled);
  }
  return anyDataFilled;
...
static bool FillActualSoundingData2(NFmiFastQueryInfo &combinedInfo,
                                    NFmiFastQueryInfo &sourceInfo,
                                    bool reverseLevels)
{
  if (reverseLevels)
...
static bool TrySpecialDewPointCase(NFmiFastQueryInfo &combinedInfo,
                                   NFmiFastQueryInfo &sourceInfo,
                                   bool reverseLevels)
{
...
static bool TrySpecialWindCase(NFmiFastQueryInfo &combinedInfo,
                               NFmiFastQueryInfo &sourceInfo,
                               bool reverseLevels)
{
  bool anyDataFilled = false;
  if (combinedInfo.Param().GetParamIdent() == kFmiTotalWindMS &&
      !sourceInfo.Param(kFmiTotalWindMS) && sourceInfo.Param(kFmiWindSpeedMS))
  {
...
// Oletus, source ja combined infot on kohdillaan ja nyt vain siirret‰‰n data jokaiselta
// parametrilta ja levelilt‰.
// Jos yksik‰‰n asetettu data ei ollut missing, palautetaan true, muuten false.
static bool FillActualSoundingData(NFmiFastQueryInfo &combinedInfo,
                                   NFmiFastQueryInfo &sourceInfo,
                                   bool reverseLevels)
{
  bool anyDataFilled = false;
...
// Luotausdatat pit‰‰ yhdist‰‰ kokonaisia yhdest‰ l‰hteest‰ ja vain yhdest‰ l‰hteest‰
// (params+levels).
...
static bool FillSoundingData(NFmiFastQueryInfo &combinedInfo, NFmiFastQueryInfo &sourceInfo)
{
  if (sourceInfo.Location(combinedInfo.Location()->GetIdent()))
  {
    ...
    {
      bool reverseLevels =
          combinedInfo.PressureParamIsRising() != sourceInfo.PressureParamIsRising();
...
static void FillCombinedData(NFmiFastQueryInfo &combinedInfo,
                             NFmiFastQueryInfo &highResolutionInfo,
                             NFmiFastQueryInfo &lowResolutionInfo)
{
...
// Tarkoitus on yhdist‰‰ high-res data, jossa on v‰hemm‰n asemia ja aikoja normaaliin dataan, miss‰
// on kaikkea enemm‰n.
static unique_ptr<NFmiQueryData> CombineData(unique_ptr<NFmiQueryData> &&highResolutionData,
                                             const string &lowResolutionDataPath)
{
...
void Domain(int argc, const char *argv[])
{
  ...
  {
    ::MakeErrorAndUsageMessages(
        "Error: Invalid command line argument count, must have 2 parameters (qdin qdout)");
  }
