...
#pragma warning(disable : 4996)  // winkkari puolella fopen -funktio koetaan turvattomaksi ja siitä
                                 // tulee varoituksia (MSVC++ kääntäjän 4996)
#endif
                                 ...
                                 std::vector<double> &theValueArray)
{
  ...
  const auto &pinfo = a.SpatialReference().projInfo();
  auto plat = pinfo.getDouble("o_lat_p");
  auto plon = pinfo.getDouble("o_lon_p");
  if (!plat || !plon)
                                ...
                                std::vector<double> &theValueArray)
{
  ...
  auto clon = pinfo.getDouble("lon_0");
  auto clat = pinfo.getDouble("lat_0");
  auto tlat = pinfo.getDouble("lat_ts");
...
{
  // Parametrin nimeä on kai turha asetella, ei mene mihinkään oikeasti?!?!?
...
// asettaa gribiin level-tiedot. Jos on annettu komento riviltä over-ride level tieto, otetaan arvot
// theWantedLevel-oliosta, muuten dataan liittyvästä level-oliosta theLevelFromData.
// Jos theWantedLevel-olion levelType on 0 (=missing arvo), silloin käytetään datasta saatua
// leveliä.
...
int run(const int argc, char *argv[])
{
  ...
  {
    qi.First();
    std::vector<double> valueArray;  // tätä vektoria käytetään siirtämään dataa querydatasta
  ...
  {
    if (out)
      fclose(out);  // suljetaan outputfile myös virhe tilanteessa
