// synop2qd.cpp Tekijä Marko 28.11.2006
// Lukee halutut tiedostot/tiedostonimi-filtterit, joista
// löytyvät SYNOP-koodit tulkitaan ja niistä muodostetaan
// querydata, missä on yhdistettynä kaikki tulkitut synop-havainnot.
...
class ExceptionSynopEndOk
{
 public:
  ...
  {  // g++ kääntäjän takia pitää tehdä dummy konstruktori, jolle annetaan parametri
  }
};
                  ...
                  const std::string &theSynopStr)
{
  ...
  // Tarkistuksia että haettu ali stringi pysyy annetun stringin sisällä
...
static float SynopVisCode2Metres(float theVisibility)
{
  if (theVisibility == kFloatMissing)
    ...
    return 75;  // 75 metriä on arvaus, 00-koodi on alle 100 m
    ...
    return 75000;  // 75000 metriä on arvaus, 89-koodi on yli 70000 m
  else if (theVisibility >= 51 && theVisibility <= 55)
    return 5000;  // sellaisia synoppeja on pilvin pimein, missä on VV -koodi 51-55, tein niin että
                  // niistä ei valiteta ja arvoksi laitetaan pyöreät 5000 m
...
class NFmiSynopCode
{
 public:
  ...
  // puretaan 4PPPP-kenttä // tämä on meren pintaan redukoitu paine
  // paitsi jos ryhmä 3 oli annettu, ja tässä onkin 4a3hhh
  ...
  void DecodeBouy(const std::string &theSynopStr, bool fWindSpeedInKnots, int theWordSkipCount)
  {
    try
    {
      ...
      {  // tämä kenttä skipataan ainakin toistaiseksi
      ...
      {  // so there was 111QdQx-field and 111-section
        ...
        {  // puretaan 2sTdTdTd-kenttä / 29UUU
        ...
        {  // puretaan 3P0P0P0P0-kenttä // tämä on asemalla mitattu paine // tätä ei toistaiseksi
           // oteta talteen
#if 0
    ...
    {
      // tämä poikkeus lopetus ok, poistutaan vain tästä metodista, ja laitetaan synop talteen
    }
  }

  // tämä on puuttuvien asemien listaukseen käytetty debuggaus luokka
              ...
              std::set<unsigned long> &theUnknownWmoIdsInOut)
  {
    try
    {
      ...
      if (fDoShipMessages)
      {
        ...
        itsStation.SetLatitude(itsDriftingStationLocation.Y());
        itsStation.SetLongitude(itsDriftingStationLocation.X());
        // itsStation.SetName(IIiii_Str); // laivojen tapauksessa aseman nimi annetaan tämän
      ...
      {  // puretaan 2sTdTdTd-kenttä / 29UUU
        ...
        else
        {
          ...
          {  // joskus tulee invaliideja 20/05 tms kenttiä, jotka pitää vain ohittaa
          }
        }
      ...
      {  // puretaan 3P0P0P0P0-kenttä // tämä on asemalla mitattu paine // tätä ei toistaiseksi
         // oteta talteen
				...
				{ // joskus tulee invaliideja 30/// kenttiä, jotka pitää vain ohittaa
				}
				double PPP32 = ::GetValue<double>(currentSynopField, 4, 4, theSynopStr);
      ...
      {  // puretaan 6RRRt-kenttä
        ...
        // Duration of period of precip, ei käytetä
      ...
      // käydään tarvittaessa läpi 333-sectionin kohtia
      if (currentSynopField == string("333"))
      {
        // luetaan 333:sta seuraava kenttä
        commentString = "next trying to read 1SnTxTxTx-field from 333-section";
        ReadNextField(ssin, currentSynopField, commentString, theSynopStr, false);

        if (currentSynopField.size() == 5 && currentSynopField[0] == '1')
        {  // puretaan 1SnTxTxTx-kenttä
  ...
  std::string itsCorrectionField;  // Jos sanoma kuuluu johonkin korjaus blokkiin, tähän tulee
...
static NFmiLocationBag MakeLocationBag(std::set<NFmiStation> &theStations, bool fDoShipMessages)
{
  unsigned long shipMessageStationId =
      122000;  // tämä on vain jokin alkuid arvo jota kasvatetaan jokaiselle eri laivalle
  ...
  for (int index = 0; it != theStations.end(); ++it, index++)
  {
    NFmiStation &station = const_cast<NFmiStation &>(
        *it);  // g++ kääntäjän takia pitää tehdä tälläisiä const_cast kikkkailuja
    ...
    if (s1 == s2)
    {
      ...
      station.SetIdent(
          shipMessageStationId++);  // ekalle ship asemalle pitää tehdä id muunnos väkisin
                                          ...
                                          const NFmiMetTime &referenceTime)
{
  if (theTimeStr.size() != 5)
  {
    if (!(theTimeStr.size() == 6 && ::isdigit(static_cast<unsigned char>(theTimeStr[4])) &&
          ::isdigit(static_cast<unsigned char>(theTimeStr[5]))))  // antaa tietyissä tapauksissa
      // mennä vähän virheellinen aika
      // ruhmä läpi, eli jos on 5 merkin
      // sijasta 6, mutta lopun jutut on
      // numeroita, niin menköön
      throw runtime_error(
  ...
  if (aTime > currentTime)
    aTime.PreviousMetTime(NFmiTimePerioid(
        0, 1, 0, 0, 0, 0));  // siirretään kuukausi taaksepäin, jos saatu aika oli tulevaisuudessa
  return aTime;
}
//                                                     HHmmw                       DDMMY
static NFmiMetTime GetTimeFromBuoyHeader(const string &theTimeStr, const string &theDateStr)
{
  ...
  NFmiMetTime aTime2(year, month, day, hour);  // tehdään nyt kokonais tunteja vain toistaiseksi
  if (aTime2 > currentTime)
    aTime2.PreviousMetTime(NFmiTimePerioid(
        10, 0, 0, 0, 0, 0));  // siirretään 10 vuotta taaksepäin, jos saatu aika oli tulevaisuudessa
  return aTime2;
}

static void FillMissingHumidityValues(NFmiFastQueryInfo &theInfo)
{
  if (theInfo.Param(kFmiTemperature) == false)
    return;
  unsigned long T_ind = theInfo.ParamIndex();
  if (theInfo.Param(kFmiDewPoint) == false)
    return;
  unsigned long Td_ind = theInfo.ParamIndex();
  if (theInfo.Param(kFmiHumidity) == false)
    return;
...
{
  // tiedetään että sanomiin voi päästä mm. 3D sana alkuun, joka pitää vain ottaa pois synop
  // sanomasta
  string badWord("3D");
                                          ...
                                          const std::string &theCorrectionField)
{
  ...
  string dateStr;
  if (fDoBuoyMessages)
    ssin >> dateStr;  // YYMMJ  (YY päivä, MM on kuukausi ja J on vuosi (2007 = 7))
  string timeStr;
  if (doNormalTime)
  ...
  // timeStrin sana pitää vielä skipata alusta
  ...
  // synopit on eroteltu =-merkeillä, joten irroitetaan ne toisistaan
  ...
  for (int i = 0; i < static_cast<int>(codeParcels.size()); i++)
  {
    try
    {
      ...
      if ((fDoShipMessages || fDoBuoyMessages) && i > 0)
      {
        ...
        string dateStr2;
        if (fDoBuoyMessages)
          ssin >> dateStr2;  // YYMMJ  (YY päivä, MM on kuukausi ja J on vuosi (2007 = 7))
        string timeStr2;
        ...
        if (shipName == shipName2 && timeStr == timeStr2)
          continue;  // nämä on jotain ihme monesti roportissa olevia samoilta laivoilta tulevia
                     // sanomia
        shipName = shipName2;
        aTime = aTime2;
        wordSkipCount =
            fDoBuoyMessages
                ? 3
                : 2;  // tässä tapauksessa Decode-metodiasa pitää skipata kaksi ensimmäistä sanaa
      }
      if (fDoBuoyMessages)
      ...
      synopCode.itsTime = aTime;  // tässä blokissa on synopit aina samassa ajassa
      if (fDoShipMessages || fDoBuoyMessages)
    ...
    {
      // tämä oli normaali synop koodin lopetus, eli ei raporttia, mutta ei laiteta talteen
...
// Oletus: stringistä on poistettu turhat whitespacet alusta, lopusta ja mahdollisten sanojen
// välillä on aina vain yksi space.
// pair:in first:issa on toiseksi viimeinen sana ja second:issa on viimeinen sana (myös
    ...
    if (StringContainsOnlyDigits(timeStr))
    {
      ...
      if (day > 0 && day <= 31 && hour >= 0 && hour < 24 && minute >= 0 && minute < 60)
      {
        ...
        if (theSynopTime > currentTime)
          theSynopTime.PreviousMetTime(NFmiTimePerioid(0, 1, 0, 0, 0, 0));  // siirretään kuukausi
        // taaksepäin, jos saatu
        // aika oli
        // tulevaisuudessa
...
static bool GetPossibleSynopCorrectionField(const std::string &str, std::string &theCorrectionField)
{
  if (str.size() == 3)
  {
    if (str[0] == str[1])  // Kahden ensimmäisen merkin pitää olla samoja esim. RRA (tai mitä muita
                           // korjaus kentti tunnisteita voi olla)
    {
      if (std::isalpha(str[0]) && std::isalpha(str[2]))  // Ensimmäisen ja viimeisen merkin pitää
                                                         // olla joku kirjain (tällöin myös 2. on
                                                         // kirjain)
      {
...
// tiedoston teksti jaetaan osiin niiden mukaan, käy seuraavaa:
// Edellisen lohkon lopussa voi olla tietoa seuraavalle lohkolle. Kaksi viimeistä sanaa voi olla
// seuraavan lohkon aika ja mahdollinen RRA (tai muu vastaava?) tieto.
// Siksi kun yksi lohko on käyty läpi, pitää lopusta ottaa tiedot talteen seuraavalle lohkolle.
...
{
  // AAXX = maa havainto ja BBXX = poiju/laiva, jotka skipataan tässä vaiheessa
  ...
  for (int j = 0; j < static_cast<int>(aaCodeBlocks.size());
       j++)  // Huom! 1. stringiä ei oteta, koska se on turha headeri osa
  {
    ...
    usedSynopBlock =
        ::TrimFromExtraSpaces(usedSynopBlock);  // pitää trimmata alku ja loppu spaceista, muuten
                                                // aika stringin sijainti laskut voivat mennä
                                                // pieleen, kun reallyUsedSynopBlock-stringiä
                                                // lasketaan
    const size_t kMaxErrorStrSize = 100;  // tämän verran laitetaan synop-blockin alusta cerr:iin
    try
    {
                     ...
                     NFmiQueryData *theData)
{
  ...
  {  // sitten täytetään uusi data
    ...
    // kun synop data on täytetty, lasketaan mahd. puuttuva kosteus arvo dataan T ja Td avulla
...
void Domain(int argc, const char *argv[])
{
  ...
  for (unsigned int j = 0; j < fileFilterList.size(); j++)
  {
    ...
    {
      //	3. Lue listan tiedostot vuorollaan sisään ja tulkitse siitä sanomat
      // synopCode-vektoriin
