...
void NcFileExtended::copy_values(const Options &options, NcVar *var, NFmiFastQueryInfo &info)
{
  ...
  if (options.debug)
    std::cerr << "debug: starting copy for variable " << name << std::endl;
...
NcFileExtended::NcFileExtended(const std::string &path,
                               long timeshift,
                               FileMode fm,
                               size_t *bufrsizeptr,
                               size_t initialsize,
 ...
 */
NFmiTimeList NcFileExtended::timeList(const std::string &varName, const std::string &unitAttrName)
{
...
void NcFileExtended::parse_time_units(boost::posix_time::ptime *origintime, long *timeunit) const
{
  ...
  *origintime = Fmi::TimeParser::parse(datestr + " " + timestr);
  ...
  if (parts.size() == 5 && !boost::iequals(parts[4], "UTC"))
    ...
    NcVar *var, int n, double &x1, double &x2, const char *name, bool &isdescending) const
{
  ...
  {
    if (!isdescending && values->as_double(i) <= values->as_double(i - 1))
...
std::shared_ptr<std::string> NcFileExtended::get_axis_units(NcVar *axis)
{
  ...
  if (att == nullptr)
    throw Fmi::Exception(BCP,
                         (std::string)axis->name() + (std::string) "-axis has no units attribute");
